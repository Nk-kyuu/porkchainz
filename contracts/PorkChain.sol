...
contract PorkChain {
   struct Pig {
    ...
    Pig[] public pigs;
    mapping(bytes32 => Pig) public pigsByHash;
    mapping(uint8 => bytes32) public pigHashes;
    ...
    function addPig(uint8 _pigWeight, string memory _pigHealth) public {
        pigCount++;
        bytes32 pigHash = keccak256(abi.encodePacked(pigCount, _pigWeight, _pigHealth));
        pigsByHash[pigHash] = Pig(pigCount, _pigWeight, _pigHealth);
        pigHashes[pigCount] = pigHash;
        pigs.push(Pig(pigCount, _pigWeight, _pigHealth));
        emit PigAdded(pigCount, _pigWeight, _pigHealth);
    }

    function getPigDataByHash(bytes32 pigHash) public view returns (uint8, uint8, string memory) {
        Pig storage pig = pigsByHash[pigHash];
        return (pig.pigID, pig.pigWeight, pig.pigHealth);
    }
}
